...
Here `|` means _or_. The order in which the grammar tries the rules is not
defined theorically, but in our imlementation we'll use a top-down approach.

Our tiny language `L` is a _formal language_, because that's what we call
languages which were generated by a grammar. That's a good thing, because we
want our language to have no ambiguity. We'll learn why in a few sections below.

Also, we won't write just any grammar, we'll limit ourselves in the rules our
grammar includes in order to match only Context-Free languages. Basically, we
only match a subset of all possible languages because context-free grammars
(CFG) represent the best compromise between power of expression and ease of
implementation. [You can learn more about grammars
here](http://www.cs.nuim.ie/~jpower/Courses/Previous/parsing/node21.html).

What limits, you might ask? Well, not much, we just need to avoid
...
limitation? Well, one is because it's harder to implement. We'll use functions
to implement rules, so because each rule gets it's own function, `Foo` now looks
like this:
...
Let's see a grammar a bit more useful. Consider this grammar:
...
If we were to manually try out this grammar, we can use the substitution model
to try it out. All we have to do is replace rules with their body until all
we have are terminals.

```
# I want to match "foo = 1"
Assign = Identifier EQUALS Number
Assign = WORD EQUALS NUMBER
Assign = foo EQUALS NUMBER # foo is a valid workd token
...
You might notice because words can be arbitrary large, that our language is
infinite. This means, it has inifinite members. In reality we don't have
infinite memory so we won't be able to match ridiculously long words, but that's
a known limitation of [turing machine](https://en.wikipedia.org/wiki/Turing_machine)
implementations (eg: Computers).
